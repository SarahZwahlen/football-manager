  ...
  return (
    <div>
      <form className="small-form">
        ...
        <div>
          <label>Nom</label>
          <input value={newPlayerData?.name} onChange={(e) => setNewPlayerData({...newPlayerData, name: e.target.value})} />
          <label>Poste</label>
          <select onChange={(e) => setNewPlayerData({...newPlayerData, playerPosition: e.target.value})}>
            <option value="Attaquant">Attaquant</option>
            <option value="Défenseur">Défenseur</option>
            <option value="Gardien">Gardien</option>
          </select>
          <label>Age</label>
          <input type="number" value={newPlayerData?.age} min="1" required onChange={(e) => setNewPlayerData({...newPlayerData, age: e.target.value>40 ? null : parseFloat(e.target.value)})}/>
          <label>Titulaire</label>
          <select onChange={(e) => setNewPlayerData({...newPlayerData, isStarterPlayer : e.target.value ==='true' ? true : false})}>
